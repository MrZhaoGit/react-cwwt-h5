...
import { WingBlank, Flex, Icon, InputItem, Button, ActionSheet, SwipeAction } from 'antd-mobile'
...
class IFlight extends React.Component {
  constructor(props){
    super(props)
    this.state = {
      isRTType: false, // 是否往返
  ...
  selSearchKeyFn(e){
    const that = this
    const buttonArr = that.state.selSearchKey.map((item)=> item.searchName)
    buttonArr.push("取消")
    ActionSheet.showActionSheetWithOptions({
      options: buttonArr,
      cancelButtonIndex: buttonArr.length - 1,
    ...
    (buttonIndex) => {
      const getData = [...that.state.selSearchKey]
      that.setState({
        selSearchKey: getData.map((item, idx) => idx === buttonIndex ?{...item,isSelected: true}: {...item, isSelected: false})
      })
    });
  }
  // 选择出发/到达城市
  ...
  render (){
    ...
    for(let i = 0; i<this.state.selSearchKey.length; i++){
      if(this.state.selSearchKey[i].isSelected){
        selSearchName = this.state.selSearchKey[i].searchName
      }
    }
          ...
          <ul className="FS_content FS_content_FL" style={!showStyle?{"display": "none"}:{}}>
            ...
            <li>
              <InputItem
                className="searchMsg FeeType"
                editable = {false}
                defaultValue={selSearchName === ""?"请选择查询条件":selSearchName}
                value={selSearchName}
                onClick={this.selSearchKeyFn.bind(this)}
              ><div>查询条件</div></InputItem>
            </li>
          </ul>
...
export default connect(mapStateToProps)(IFlight)